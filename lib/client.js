// dsh-auto-review — client half (v1.6.0；v1.6 client 批次增量見下).
//
// v1.6 client 增量（P1-13 UI / P1-14 / P1-15）：
//   P1-14 報告渲染升級——MdRender 標題階梯/表格斑馬紋+嚴重度色籤/行內 code 底色/引用豎線強調；
//     報告頂部摘要卡（狀態/輪次/token in·out/阻斷計數）＋維度錨點 TOC（點擊滾動）＋
//     維度章節摺疊（默認：未通過展開、已通過收起）＋「複製 MD」（navigator.clipboard→textarea
//     降級）與「下載 .md」（anchor download + data URL）。
//   P1-15 統計圖表（純手寫 SVG/CSS 零依賴；SVG 帶 aria-label+文字備援）：嚴重度環形
//     （stroke-dasharray）／輪次收斂折線（polyline+節點數值，空/單點安全）／維度×嚴重度
//     堆疊橫條／檔案熱點 Top5／頑固項清單（file+normTitle 跨輪≥2 且未解決）。數據自
//     publicRun/roundLog/injectLog 客戶端自算；位置：報告頂部「統計摘要」段＋面板「統計」摺疊區。
//   P1-13 UI 歷史記錄——面板「歷史記錄」摺疊區：清單 API 時間倒序卡（日期/狀態/輪次/模型/
//     阻斷），點擊載入明細 → 舊報告+統計只讀渲染（「歷史歸檔」標注；空/載入/失敗/未啟用態齊備）。
//
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function callApi(method, args) {
			...
			// v1.6（P1-13 UI）：審查歷史——清單與明細（鑑權與重試與其他 GET 端點一致）
			if (method === "review-history-list") {
				const hp = encodeURIComponent(String((args || {}).project || ""));
				return withToken("/__review/api/history" + (hp !== "" ? "?project=" + hp : ""), {});
			}
			if (method === "review-history-get") {
				const ha = args || {};
				let hurl = "/__review/api/history/" + encodeURIComponent(String(ha.runId || ""));
				const hp2 = encodeURIComponent(String(ha.project || ""));
				if (hp2 !== "") hurl += "?project=" + hp2;
				return withToken(hurl, {});
			}
		...
		/** P1-14：短延遲（「已複製 ✓」回彈）——動態模式 ctx.timer，bundle 直用 setTimeout。 */
		function delay(fn, ms) {
			if (pluginCtx) return later(fn, ms);
		...
		/** 行內解析：**粗體** 與 `行內 code`（P1-14：code 帶底色等寬字體）；其他字符原樣輸出。 */
		function renderInline(text, keyPrefix) {
			...
			while ((m = re.exec(s)) !== null) {
				...
				out.push(m[1].charAt(0) === "`"
					? e("code", { key: keyPrefix + "-c" + (n++), style: { fontFamily: mono, fontSize: 11.5, background: "rgba(88, 166, 255, 0.10)", borderRadius: 4, padding: "1px 5px" } }, m[1].slice(1, -1))
					: e("strong", { key: keyPrefix + "-b" + (n++), style: { color: cText, fontWeight: 600 } }, m[1].slice(2, -2)));
		...
		/** P1-14：標題錨點 slug（TOC 與 MdRender 共用——同文本同 id；僅保留字母數字與 CJK）。 */
		function anchorSlugOf(text) {
			return String(text ?? "").toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 48) || "s";
		}

		/** P1-14：掃描 Markdown 的 h1-h3 標題 → [{text, level, id}]（TOC 與 MdRender 錨點的
		 *  單一事實來源；同文本重複時按出現序加 -2/-3 去重）。 */
		function mdHeadingAnchors(text) {
			const out = [];
			const used = {};
			for (const line of String(text ?? "").split("\n")) {
				const m = /^(#{1,3})\s+(.*)$/.exec(line.trim());
				if (m === null) continue;
				const base = "ar-md-" + anchorSlugOf(m[2]);
				const n = (used[base] = (used[base] || 0) + 1);
				out.push({ text: m[2], level: m[1].length, id: n === 1 ? base : base + "-" + n });
			}
			return out;
		}

		/** P1-14：標題階梯樣式（h1 帶底線分隔、h2 章節頭）。 */
		function mdHeadingStyle(lv) {
			return lv === 1
				? { color: cText, fontWeight: 700, fontSize: 15.5, margin: "12px 0 8px", paddingBottom: 4, borderBottom: "1px solid " + cBorder }
				: { color: cText, fontWeight: 700, fontSize: 14, margin: "12px 0 6px" };
		}

		/** P1-7→P1-14：輕量 Markdown 渲染——標題階梯（h1/h2 帶錨點 id 供 TOC 跳轉）/粗體/行內
		 *  code 底色/引用豎線強調/清單/表格斑馬紋+嚴重度色籤/分隔線。collapsible=true 時 h2
		 *  章節可摺疊，defaultOpen(title) 決定初始態（報告視圖：未通過展開、✅ 已通過收起）。
		 *  摺疊態用單一 openMap state（章節數隨文本變化，不在循環裡掛 hooks）。 */
		function MdRender(props) {
			const src = String(props.text ?? "");
			const collapsible = props.collapsible === true;
			const anchors = mdHeadingAnchors(src);
			const idOf = {};
			for (const a of anchors) if (idOf[a.text] === undefined) idOf[a.text] = a.id;
			const openPair = useState(null);
			const openMap = openPair[0];
			const setOpenMap = openPair[1];
			const isOpen = function (title) {
				if (openMap !== null && Object.prototype.hasOwnProperty.call(openMap, title)) return openMap[title] === true;
				return props.defaultOpen ? props.defaultOpen(title) !== false : true;
			};
			const toggleSec = function (title) {
				const next = Object.assign({}, openMap || {});
				next[title] = !isOpen(title);
				setOpenMap(next);
			};
			const lines = src.split("\n");
			const sections = []; // {title, level, id, blocks}——h1/h2 開新章節；h3 為章節內塊
			let cur = { title: null, level: 0, id: null, blocks: [] };
			let bi = 0;
			...
			while (i < lines.length) {
				...
				if (h !== null) {
					const lv = h[1].length;
					if (lv <= 2) {
						if (cur.title !== null || cur.blocks.length > 0) sections.push(cur);
						cur = { title: h[2], level: lv, id: idOf[h[2]] ?? null, blocks: [] };
					} else {
						cur.blocks.push(e("div", { key: "md" + (bi++), style: { color: cText, fontWeight: 600, fontSize: 12.5, margin: "10px 0 4px" } }, renderInline(h[2], "h3" + bi)));
					}
					i++; continue;
				}
				// 表格：| 開頭 + 下一行 |---| 分隔（P1-14：奇數行斑馬紋 + severity 色籤=色點+色字）
				if (t.charAt(0) === "|" && i + 1 < lines.length
					...
					cur.blocks.push(e("table", { key: "md" + (bi++), style: { borderCollapse: "collapse", width: "100%", margin: "8px 0", fontSize: 12, tableLayout: "fixed" } },
						...
						e("tbody", null, rows.map(function (r, ri) {
							return e("tr", { key: ri, style: { background: ri % 2 === 1 ? "rgba(139, 148, 158, 0.09)" : "transparent" } }, r.map(function (c, ci) {
								...
								return e("td", { key: ci, style: { border: "1px solid " + cBorder, padding: "4px 8px", verticalAlign: "top", lineHeight: 1.5, wordBreak: "break-word", color: cText } },
									sev !== undefined
										? e("span", { style: { display: "inline-flex", alignItems: "center", gap: 6 } },
											e("span", { style: { width: 8, height: 8, borderRadius: 99, background: sev, flexShrink: 0 } }),
											e("span", { style: { color: sev, fontWeight: 700 } }, c))
				...
				// 分隔線 ---
				if (/^-{3,}$/.test(t)) {
					cur.blocks.push(e("div", { key: "md" + (bi++), style: { borderTop: "1px solid " + cBorder, margin: "10px 0" } }));
					i++; continue;
				}
				// 引用 >（P1-14：豎線強調——品牌色左線 + 淡底色）
				if (t.charAt(0) === ">") {
					const qs = [];
					while (i < lines.length && lines[i].trim().charAt(0) === ">") { qs.push(lines[i].trim().replace(/^>\s?/, "")); i++; }
					cur.blocks.push(e("div", { key: "md" + (bi++), style: { borderLeft: "3px solid " + cBrand, background: "rgba(139, 148, 158, 0.07)", borderRadius: "0 6px 6px 0", padding: "6px 10px", color: cMuted, fontSize: 12, lineHeight: 1.6, margin: "4px 0" } },
				...
				if (/^[-*]\s+/.test(t)) {
					...
					cur.blocks.push(e("ul", { key: "md" + (bi++), style: { margin: "4px 0", paddingLeft: 18, color: cText, fontSize: 12, lineHeight: 1.6 } },
				...
				cur.blocks.push(e("div", { key: "md" + (bi++), style: { color: cText, fontSize: 12, lineHeight: 1.6, margin: "4px 0" } },
					renderInline(para.join(" "), "p" + bi)));
			}
			if (cur.title !== null || cur.blocks.length > 0) sections.push(cur);
			return e("div", null, sections.map(function (sec, si) {
				if (sec.title === null) return e("div", { key: "sec" + si }, sec.blocks);
				if (collapsible && sec.level === 2) {
					const open = isOpen(sec.title);
					return e("div", { key: "sec" + si, style: { border: "1px solid " + cBorder, borderRadius: 10, background: cBg1, margin: "8px 0" } },
						e("div", {
							id: sec.id ?? undefined, role: "button", tabIndex: 0, "aria-expanded": open,
							onClick: function () { toggleSec(sec.title); },
							onKeyDown: function (ev) { if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); toggleSec(sec.title); } },
							style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", cursor: "pointer" },
						},
							e("span", { style: { color: cMuted, fontSize: 11 } }, open ? "▾" : "▸"),
							e("span", { style: mdHeadingStyle(2) }, renderInline(sec.title, "sh" + si))),
						open ? e("div", { style: { padding: "0 12px 10px" } }, sec.blocks) : null);
				}
				return e("div", { key: "sec" + si },
					e("div", { id: sec.id ?? undefined, style: mdHeadingStyle(sec.level) }, renderInline(sec.title, "h" + si)),
					sec.blocks);
			}));
		}
		...
		//#region v1.6（P1-13 UI / P1-14 / P1-15）：統計圖表 + 報告視圖 + 歷史記錄
		const SEV_ORDER = ["critical", "high", "medium", "low"];
		const SEV_RANK = { critical: 3, high: 2, medium: 1, low: 0 };
		const GATE_SEVS = { loose: ["critical"], standard: ["critical", "high"], strict: ["critical", "high", "medium"] };

		/** 終態/暫態色調（面板狀態膠囊同款 D-4 映射——歷史卡與摘要卡共用）。 */
		function statusToneOf(status) {
			return status === "paused" || status === "interrupted" ? cWarn
				: status === "failed" || status === "stopped" ? cBad
				: status === "oscillated" || status === "max-rounds" ? cWarn
				: status === "passed" || status === "reported" ? cGood : cBrand;
		}
		...
		/** P1-14：TOC 錨點滾動（僅查詢自身渲染的 id；無全局 DOM 副作用）。 */
		...
		/** P1-14：複製文本——navigator.clipboard 優先，降級 textarea+execCommand（瞬態節點，即建即刪）。 */
		...
		/** P1-15：publicRun → 統計聚合（純函數）——維度計數/嚴重度合計/輪次收斂序列/
		 *  檔案熱點（未解決 findings 按 file 聚合）/頑固項指紋（file+normTitle 跨輪注入≥2
		 *  且終態仍未解決）。數據全部來自面板已有的 run 視圖，無新 API。 */
		function statsOfRun(run) {
			const dims = [];
			const sevTotal = { critical: 0, high: 0, medium: 0, low: 0 };
			const fileMap = {};
			// 指紋正規化對齊 host normTitle：剝非字母數字 CJK；全剝空時回退原文小寫（防同檔全碰撞）
			const normTitle = function (t) {
				const s = String(t ?? "").toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, "");
				return s !== "" ? s : String(t ?? "").toLowerCase();
			};
			for (const d of (run && run.dimensions) || []) {
				const counts = { critical: 0, high: 0, medium: 0, low: 0 };
				let total = 0;
				for (const sev of SEV_ORDER) {
					counts[sev] = (d.counts || {})[sev] || 0;
					sevTotal[sev] += counts[sev];
					total += counts[sev];
				}
				dims.push({ id: d.id, label: d.label, status: d.status, counts: counts, total: total });
				for (const f of (d.findings || [])) {
					if (f.resolved === true) continue;
					const file = String(f.file || "—");
					const rec = fileMap[file] || (fileMap[file] = { count: 0, sevMax: "low", line: null });
					rec.count++;
					if ((SEV_RANK[f.severity] || 0) > (SEV_RANK[rec.sevMax] || 0)) rec.sevMax = f.severity;
					if (rec.line == null && f.line != null) rec.line = f.line;
				}
			}
			const totalFindings = sevTotal.critical + sevTotal.high + sevTotal.medium + sevTotal.low;
			const rounds = ((run && run.roundLog) || []).slice()
				.sort(function (a, b) { return (a.round || 0) - (b.round || 0); })
				.map(function (x) { return { round: x.round, merged: x.mergedCount }; });
			// 頑固項指紋：injectLog 各輪 items（注入=阻斷級建議；順帶修復不計）
			const fpMap = {};
			for (const inj of (run && run.injectLog) || []) {
				for (const it of (inj.items || [])) {
					const key = String(it.file || "—") + "|" + normTitle(it.title);
					const rec = fpMap[key] || (fpMap[key] = { file: String(it.file || "—"), title: String(it.title || ""), line: it.line ?? null, sev: it.severity || "low", rounds: [] });
					if (rec.rounds.indexOf(inj.round) < 0) rec.rounds.push(inj.round);
					if ((SEV_RANK[it.severity] || 0) > (SEV_RANK[rec.sev] || 0)) rec.sev = it.severity;
					if (rec.line == null && it.line != null) rec.line = it.line;
				}
			}
			const unresolved = {};
			for (const d of (run && run.dimensions) || []) {
				for (const f of (d.findings || [])) {
					if (f.resolved === true) continue;
					unresolved[String(f.file || "—") + "|" + normTitle(f.title)] = true;
				}
			}
			const stubborn = [];
			for (const key in fpMap) {
				if (fpMap[key].rounds.length >= 2 && unresolved[key] === true) stubborn.push(fpMap[key]);
			}
			stubborn.sort(function (a, b) { return b.rounds.length - a.rounds.length; });
			const fileTop = [];
			for (const file in fileMap) fileTop.push(Object.assign({ file: file }, fileMap[file]));
			fileTop.sort(function (a, b) { return b.count - a.count; });
			// t4-m 修復：阻斷數對齊 host mergedBlockingCount 口徑——gate 內未解決 → 排除 .reviewignore
			// 已接受（file+normTitle 指紋）→ 跨維度指紋合併（同指紋行號聚類跨度 ≤10 行合一條）。
			// 與歷史卡（host 清單 r.blocking）同數：命中 ignore 或跨維重複時不再虛計「不阻擋驗收」的項。
			const gateSet = {};
			for (const s of (GATE_SEVS[(run && run.gate)] || GATE_SEVS.standard)) gateSet[s] = true;
			const ignoredFps = {};
			for (const arr of Object.values((run && run.ignoredByDecision) || {})) {
				for (const f of arr || []) ignoredFps[String(f.file || "") + "|" + normTitle(f.title)] = true;
			}
			const fpGroups = {};
			for (const d of (run && run.dimensions) || []) {
				for (const f of (d.findings || [])) {
					if (f.resolved === true || !gateSet[f.severity]) continue;
					const fp = String(f.file || "") + "|" + normTitle(f.title);
					if (ignoredFps[fp]) continue;
					const n = Number(f.line);
					(fpGroups[fp] || (fpGroups[fp] = [])).push(Number.isFinite(n) && n >= 0 ? n : null);
				}
			}
			let blockingMerged = 0;
			for (const lines of Object.values(fpGroups)) {
				const clusters = [];
				for (const ln of lines) {
					let target = null;
					if (ln === null) target = clusters.length > 0 ? clusters[0] : null;
					else {
						for (const c of clusters) {
							const cmin = c[0] === null ? ln : Math.min(c[0], ln);
							const cmax = c[1] === null ? ln : Math.max(c[1], ln);
							if (cmax - cmin <= 10) { target = c; break }
						}
					}
					if (target === null) clusters.push([ln, ln]);
					else if (ln !== null) {
						target[0] = target[0] === null ? ln : Math.min(target[0], ln);
						target[1] = target[1] === null ? ln : Math.max(target[1], ln);
					}
				}
				blockingMerged += clusters.length;
			}
			return {
				dims: dims, sevTotal: sevTotal, totalFindings: totalFindings,
				rounds: rounds, fileTop: fileTop.slice(0, 5), stubborn: stubborn.slice(0, 10),
				blockingMerged: blockingMerged,
				hasInjectLog: ((run && run.injectLog) || []).length > 0,
				hasData: rounds.length > 0 || totalFindings > 0,
			};
		}

		/** P1-15①：嚴重度環形（SVG stroke-dasharray；中間總數；圖例=文字備援）。 */
		function SevDonut(props) {
			const counts = props.counts || {};
			const total = Number(props.total) || 0;
			if (total <= 0) return e("div", { style: { color: cMuted, fontSize: 12 } }, "無未解決發現");
			...
			for (const sev of SEV_ORDER) {
				...
				legend.push(e("span", { key: sev, style: { color: SEV_COLOR[sev], fontSize: 11, display: "inline-flex", alignItems: "center", gap: 4 } },
					e("span", { style: { width: 8, height: 8, borderRadius: 99, background: SEV_COLOR[sev], flexShrink: 0 } }),
					sev + " " + n));
				if (n <= 0) continue;
				const dash = (n / total) * C;
				segs.push(e("circle", { key: sev, cx: 60, cy: 60, r: 44, "aria-hidden": true,
					style: { fill: "none", stroke: SEV_COLOR[sev], strokeWidth: 14, strokeDasharray: dash.toFixed(2) + " " + (C - dash).toFixed(2), strokeDashoffset: (-acc).toFixed(2) } }));
				acc += dash;
			}
			return e("div", { style: { display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" } },
				e("svg", { viewBox: "0 0 120 120", width: 110, height: 110, role: "img",
					"aria-label": "嚴重度分佈環形圖，共 " + total + " 項未解決：" + SEV_ORDER.map(function (s) { return s + " " + (counts[s] || 0) + " 項"; }).join("、"),
					style: { display: "block", flexShrink: 0 } },
					e("g", { transform: "rotate(-90 60 60)" },
						e("circle", { cx: 60, cy: 60, r: 44, style: { fill: "none", stroke: cBorder, strokeWidth: 14 } }),
						segs),
					e("text", { x: 60, y: 57, style: { fill: cText, fontSize: 20, fontWeight: 700, textAnchor: "middle" } }, String(total)),
					e("text", { x: 60, y: 73, style: { fill: cMuted, fontSize: 9.5, textAnchor: "middle" } }, "項未解決")),
				e("div", { style: { display: "flex", flexDirection: "column", gap: 3 } }, legend));
		}

		/** P1-15②：輪次收斂折線（roundLog mergedCount 序列；SVG polyline+節點數值；
		 *  空/單點安全——單點只畫節點與數值）。 */
		function ConvChart(props) {
			const pts = (props.points || []).filter(function (p) { return p.merged != null; });
			const aria = "輪次收斂折線：" + (pts.map(function (p) { return "R" + p.round + " 發現 " + p.merged + " 項"; }).join("、") || "無數據");
			if (pts.length === 0) return e("div", { style: { color: cMuted, fontSize: 12 } }, "尚無輪次數據（第一輪審查完成後畫出收斂折線）");
			const W = 320, H = 116, PL = 30, PR = 14, PT = 20, PB = 20;
			const max = Math.max.apply(null, pts.map(function (p) { return p.merged; }).concat([1]));
			const iw = W - PL - PR, ih = H - PT - PB;
			const xOf = function (i2) { return pts.length === 1 ? PL + iw / 2 : PL + (iw * i2) / (pts.length - 1); };
			const yOf = function (v) { return PT + ih - (ih * v) / max; };
			const poly = pts.map(function (p, i2) { return xOf(i2).toFixed(1) + "," + yOf(p.merged).toFixed(1); }).join(" ");
			return e("svg", { viewBox: "0 0 " + W + " " + H, width: "100%", role: "img", "aria-label": aria,
				style: { maxWidth: 360, display: "block" } },
				e("line", { x1: PL, y1: PT, x2: PL, y2: PT + ih, style: { stroke: cBorder, strokeWidth: 1 } }),
				e("line", { x1: PL, y1: PT + ih, x2: W - PR, y2: PT + ih, style: { stroke: cBorder, strokeWidth: 1 } }),
				e("text", { x: PL - 4, y: PT + 4, style: { fill: cMuted, fontSize: 9, textAnchor: "end" } }, String(max)),
				e("text", { x: PL - 4, y: PT + ih + 3, style: { fill: cMuted, fontSize: 9, textAnchor: "end" } }, "0"),
				pts.length > 1 ? e("polyline", { points: poly, style: { fill: "none", stroke: cBrand, strokeWidth: 2 } }) : null,
				pts.map(function (p, i2) {
					return e("g", { key: "r" + p.round },
						e("circle", { cx: xOf(i2), cy: yOf(p.merged), r: 3.2, style: { fill: cBrand } }),
						e("text", { x: xOf(i2), y: yOf(p.merged) - 7, style: { fill: cText, fontSize: 10, textAnchor: "middle" } }, String(p.merged)),
						e("text", { x: xOf(i2), y: PT + ih + 13, style: { fill: cMuted, fontSize: 9, textAnchor: "middle" } }, "R" + p.round));
				}));
		}

		/** P1-15③：維度×嚴重度堆疊橫條（flex 段寬=計數；段 title 帶明細）。 */
		function DimStackChart(props) {
			const dims = props.dims || [];
			if (dims.length === 0) return e("div", { style: { color: cMuted, fontSize: 12 } }, "無維度數據");
			if (!dims.some(function (d) { return d.total > 0; })) return e("div", { style: { color: cMuted, fontSize: 12 } }, "全部通過——無未解決項");
			return e("div", null,
				e("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 6 } },
					SEV_ORDER.map(function (sev) {
						return e("span", { key: sev, style: { color: SEV_COLOR[sev], fontSize: 11, display: "inline-flex", alignItems: "center", gap: 4 } },
							e("span", { style: { width: 8, height: 8, borderRadius: 99, background: SEV_COLOR[sev] } }), sev);
					})),
				dims.map(function (d) {
					return e("div", { key: d.id, style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 5 } },
						e("span", { style: { color: cText, fontSize: 12, width: 64, flexShrink: 0 } }, d.label),
						e("div", { style: { flex: 1, display: "flex", height: 14, borderRadius: 7, overflow: "hidden", background: "rgba(139, 148, 158, 0.12)", minWidth: 120 } },
							SEV_ORDER.map(function (sev) {
								const n = d.counts[sev] || 0;
								if (n <= 0) return null;
								return e("div", { key: sev, title: sev + " ×" + n, style: { flex: n, minWidth: 3, background: SEV_COLOR[sev] } });
							})),
						e("span", { style: { color: cMuted, fontSize: 11, width: 26, textAlign: "right", flexShrink: 0 } }, String(d.total)));
				}));
		}

		/** P1-15④：檔案熱點 Top5（未解決 findings 按 file 聚合的橫條；bar 色=該檔最高嚴重度）。 */
		function FileHotChart(props) {
			const top = props.fileTop || [];
			if (top.length === 0) return e("div", { style: { color: cMuted, fontSize: 12 } }, "無檔案熱點（沒有未解決發現）");
			const max = top[0].count;
			return e("div", { "aria-label": "檔案熱點：" + top.map(function (r) { return r.file + " " + r.count + " 項"; }).join("、") },
				top.map(function (r) {
					return e("div", { key: r.file, style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 5 } },
						e("span", { title: r.file, style: { color: cMuted, fontFamily: mono, fontSize: 11, width: 150, flexShrink: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" } }, r.file),
						e("div", { style: { flex: 1, height: 12, borderRadius: 6, background: "rgba(139, 148, 158, 0.12)", minWidth: 80 } },
							e("div", { style: { width: Math.max(4, (r.count / max) * 100) + "%", height: 12, borderRadius: 6, background: SEV_COLOR[r.sevMax] || cMuted } })),
						e("span", { style: { color: cText, fontSize: 11, width: 40, textAlign: "right", flexShrink: 0 } }, r.count + " 項"));
				}));
		}

		/** P1-15⑤：頑固項清單（file+normTitle 跨輪注入≥2 且終態仍未解決——「哪些問題修不掉」）。 */
		function StubbornList(props) {
			const items = props.items || [];
			if (items.length === 0) {
				return e("div", { style: { color: cMuted, fontSize: 12 } },
					props.hasInject ? "無跨輪重複項——注入過的建議沒有在後續輪反覆出現" : "尚無注入數據（報告模式或尚未注入——頑固項需要跨輪注入指紋）");
			}
			return e("div", null, items.map(function (r, i) {
				const rounds = (r.rounds || []).slice().sort(function (a, b) { return a - b; });
				const sev = SEV_COLOR[r.sev] || cMuted;
				return e("div", { key: i, style: { display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap", borderLeft: "3px solid " + sev, paddingLeft: 10, marginBottom: 6 } },
					e("span", { style: { color: sev, fontWeight: 700, fontSize: 11, minWidth: 46 } }, r.sev),
					e("span", { style: { color: cMuted, fontFamily: mono, fontSize: 11 } }, r.file + (r.line != null ? ":" + r.line : "")),
					e("span", { style: { color: cText, fontSize: 12, flex: 1, minWidth: 140 } }, r.title),
					e("span", { style: { color: cWarn, fontSize: 11 } }, rounds.map(function (x) { return "R" + x; }).join(" · ") + " 重複出現"),
					e("span", { style: { color: cBad, fontSize: 11, border: "1px solid " + cBad, borderRadius: 999, padding: "0 6px", lineHeight: "16px" } }, "仍未解決"));
			}));
		}

		/** P1-15：統計摘要組合（報告頂部與面板「統計」摺疊區共用）。 */
		function StatsSummary(props) {
			const run = props.run;
			if (!run || typeof run !== "object") return null;
			const st = statsOfRun(run);
			if (!st.hasData) return e("div", { style: { color: cMuted, fontSize: 12, lineHeight: 1.6 } },
				"尚無統計數據——閉環開始後這裡顯示嚴重度環形、輪次收斂折線、維度堆疊條、檔案熱點與頑固項。");
			const title = { color: cText, fontSize: 12, fontWeight: 600, margin: "12px 0 6px" };
			return e("div", null,
				e("div", { style: { display: "flex", gap: 18, flexWrap: "wrap", alignItems: "flex-start" } },
					e("div", null,
						e("div", { style: { color: cMuted, fontSize: 11, marginBottom: 4 } }, "嚴重度分佈（未解決）"),
						e(SevDonut, { counts: st.sevTotal, total: st.totalFindings })),
					e("div", { style: { flex: 1, minWidth: 260 } },
						e("div", { style: { color: cMuted, fontSize: 11, marginBottom: 4 } }, "輪次收斂（每輪合併後發現數）"),
						e(ConvChart, { points: st.rounds }))),
				e("div", { style: title }, "維度 × 嚴重度（未解決）"),
				e(DimStackChart, { dims: st.dims }),
				e("div", { style: title }, "檔案熱點 Top 5（未解決）"),
				e(FileHotChart, { fileTop: st.fileTop }),
				e("div", { style: title }, "頑固項（跨輪注入 ≥2 次且仍未解決）"),
				e(StubbornList, { items: st.stubborn, hasInject: st.hasInjectLog }));
		}

		/** P1-14：報告頂部摘要卡——狀態/輪次/token in·out/阻斷計數緊湊橫排。
		 *  t4-m 修復：阻斷改用 st.blockingMerged（host mergedBlockingCount 同口徑——排除
		 *  .reviewignore 已接受項 + 跨維度指紋合併），與歷史卡數字一致；無 ignore/跨維重複時
		 *  與「強度檔位直加」等價。 */
		function RunSummaryCard(props) {
			const run = props.run || {};
			const st = statsOfRun(run);
			const blocking = st.blockingMerged || 0;
			const cell = { color: cMuted, fontSize: 11 };
			return e("div", { style: { background: cBg1, border: "1px solid " + cBorder, borderRadius: 10, padding: "8px 12px", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" } },
				e(Pill, { tone: statusToneOf(run.status) }, RUN_LABEL[run.status] || String(run.status || "—")),
				e("span", { style: { color: cText, fontSize: 12, fontWeight: 600 } }, "R" + (run.round ?? "—") + "/" + (run.maxRounds ?? "—")),
				e("span", { style: cell }, "阻斷 " + blocking + "（強度 " + (run.gate || "standard") + "）"),
				run.tokenUsage ? e("span", { style: cell }, "token " + fmtTok(run.tokenUsage.input) + " in · " + fmtTok(run.tokenUsage.output) + " out") : null,
				run.models && run.models.length ? e("span", { style: cell }, run.models.join(" · ")) : null,
				e("span", { style: { flex: 1 } }),
				SEV_ORDER.map(function (sev) {
					return e("span", { key: sev, style: { color: SEV_COLOR[sev], fontSize: 11, fontWeight: 600 } }, sev + " " + (st.sevTotal[sev] || 0));
				}));
		}

		/** P1-14：報告視圖——動作列（複製 MD/下載 .md）＋摘要卡＋統計摘要＋維度 TOC（錨點滾動）
		 *  ＋摺疊 MdRender（未通過維度默認展開、✅ 已通過收起）。當前報告與歷史歸檔共用。 */
		function ReportView(props) {
			const run = props.run || null;
			const text = String(props.text ?? "");
			const copiedPair = useState(false);
			const copied = copiedPair[0];
			const setCopied = copiedPair[1];
			const toc = mdHeadingAnchors(text).filter(function (a) { return a.level === 2; });
			const filename = props.filename || ("review-report-" + ((run && run.runId) || "report") + ".md");
			return e("div", null,
				e("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 8 } },
					props.badge ? e(Pill, { tone: cWarn }, props.badge) : null,
					e("span", { style: { flex: 1 } }),
					e("button", {
						style: miniBtn(), onClick: function () {
							copyText(text, function () {
								setCopied(true);
								delay(function () { setCopied(false); }, 1800);
							});
						},
					}, copied ? "已複製 ✓" : "⧉ 複製 MD"),
					e("a", {
						...
						download: filename,
						style: Object.assign({}, miniBtn(), { textDecoration: "none", display: "inline-flex", alignItems: "center" }),
					}, "⬇ 下載 .md")),
				run ? e(RunSummaryCard, { run: run }) : null,
				run ? e("div", { style: { marginTop: 10 } }, e(StatsSummary, { run: run })) : null,
				toc.length >= 2 ? e("div", { style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", margin: "10px 0 6px" } },
					e("span", { style: { color: cMuted, fontSize: 11 } }, "章節："),
					toc.map(function (a) {
						return e("button", {
							key: a.id, onClick: function () { scrollToId(a.id); },
							style: { background: "transparent", color: cText, border: "1px solid " + cBorder, borderRadius: 999, padding: "1px 10px", fontSize: 11, cursor: "pointer", fontFamily: "inherit" },
						}, a.text);
					})) : null,
				e(MdRender, { text: text, collapsible: true, defaultOpen: function (t) { return String(t).indexOf("✅") < 0; } }));
		}

		/** v1.6：通用摺疊區（面板「統計」「歷史記錄」共用；鍵盤可訪問 + aria-expanded）。 */
		function CollapsibleSection(props) {
			...
			const toggle = function () {
				const nv = !open;
				setOpen(nv);
				if (nv && props.onOpen) props.onOpen();
			};
			return e("div", { style: { background: cBg1, border: "1px solid " + cBorder, borderRadius: 10, padding: "8px 12px", marginTop: 8 } },
				e("div", {
					role: "button", tabIndex: 0, "aria-expanded": open,
					onClick: toggle,
					onKeyDown: function (ev) { if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); toggle(); } },
					style: { display: "flex", alignItems: "center", gap: 8, cursor: "pointer", flexWrap: "wrap" },
				},
					e("span", { style: { color: cMuted, fontSize: 11 } }, open ? "▾" : "▸"),
					e("span", { style: { color: cText, fontSize: 13, fontWeight: 600 } }, props.title),
					props.subtitle ? e("span", { style: { color: cMuted, fontSize: 11 } }, props.subtitle) : null),
				open ? e("div", { style: { marginTop: 8 } }, props.children) : null);
		}

		/** P1-13 UI：歷史記錄卡（清單條目 → 點擊載入明細；日期/狀態/輪次/模型/阻斷/計數）。 */
		function historyCard(r, onOpen) {
			const sev = r.severityCounts || {};
			const parts = [];
			for (const s of SEV_ORDER) {
				if (!(sev[s] > 0)) continue;
				parts.push(e("span", { key: s, style: { color: SEV_COLOR[s], fontSize: 11, fontWeight: 600 } }, s + ":" + sev[s]));
			}
			return e("div", {
				key: r.runId, role: "button", tabIndex: 0,
				onClick: function () { onOpen(r); },
				onKeyDown: function (ev) { if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); onOpen(r); } },
				style: { background: cBg1, border: "1px solid " + cBorder, borderRadius: 10, padding: "8px 12px", marginBottom: 6, cursor: "pointer" },
			},
				e("div", { style: { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" } },
					e("span", { style: { color: cMuted, fontSize: 11, fontFamily: mono } }, fmtDT(r.archivedAt || r.endedAt || r.startedAt)),
					e(Pill, { tone: statusToneOf(r.status) }, RUN_LABEL[r.status] || String(r.status || "—")),
					r.projectName ? e("span", { style: { color: cText, fontSize: 12, fontFamily: mono } }, r.projectName) : null,
					e("span", { style: { flex: 1 } }),
					parts,
					r.blocking != null ? e("span", { style: { color: (r.blocking || 0) > 0 ? cBad : cGood, fontSize: 11, fontWeight: 600 } }, "阻斷 " + r.blocking) : null),
				e("div", { style: { display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap", marginTop: 3 } },
					e("span", { style: { color: cMuted, fontSize: 11 } }, "R" + (r.round ?? "—") + "/" + (r.maxRounds ?? "—")),
					Array.isArray(r.models) && r.models.length ? e("span", { style: { color: cMuted, fontSize: 11 } }, r.models.join(" · ")) : null,
					r.injectCount != null ? e("span", { style: { color: cMuted, fontSize: 11 } }, "注入 " + r.injectCount + " 輪") : null,
					r.tokenUsage ? e("span", { style: { color: cMuted, fontSize: 11 } }, "token " + fmtTok(r.tokenUsage.input + r.tokenUsage.output)) : null,
					r.truncated === true ? e("span", { style: { color: cWarn, fontSize: 11 } }, "已截斷") : null));
		}

		/** P1-13 UI：歷史記錄摺疊區——清單（時間倒序）→ 明細（舊報告+統計只讀渲染，標「歷史歸檔」）。
		 *  空/載入/失敗/未啟用（動態模式 disabled）四態齊備；數據走既有 token 鑑權 API。 */
		function HistorySection(props) {
			const listPair = useState(null); // null=未載入；{phase:'loading'|'ok'|'error'|'disabled', runs?, error?}
			const list = listPair[0];
			const setList = listPair[1];
			const selPair = useState(null); // null=列表態；{entry, phase:'loading'|'ok'|'error', detail?, error?}
			const sel = selPair[0];
			const setSel = selPair[1];
			const loadList = function () {
				setList({ phase: "loading" });
				callApi("review-history-list", {})
					.then(function (r) {
						if (r && r.disabled === true) { setList({ phase: "disabled" }); return; }
						if (!r || r.ok !== true) { setList({ phase: "error", error: (r && r.error) || "未知錯誤" }); return; }
						setList({ phase: "ok", runs: r.runs || [] });
					})
					.catch(function (ex) { setList({ phase: "error", error: String(ex && ex.message || ex) }); });
			};
			const openDetail = function (entry) {
				setSel({ entry: entry, phase: "loading" });
				callApi("review-history-get", { runId: entry.runId, project: entry.project })
					.then(function (r) {
						if (!r || r.ok !== true) { setSel({ entry: entry, phase: "error", error: (r && r.error) || "載入失敗" }); return; }
						setSel({ entry: entry, phase: "ok", detail: r });
					})
					.catch(function (ex) { setSel({ entry: entry, phase: "error", error: String(ex && ex.message || ex) }); });
			};
			let body = null;
			if (sel !== null) {
				const ent = sel.entry || {};
				if (sel.phase === "loading") {
					body = e("div", { style: { color: cMuted, fontSize: 12, padding: "6px 0" } }, "載入歸檔明細中…");
				} else if (sel.phase === "error") {
					body = e("div", { role: "alert", style: { padding: "6px 0" } },
						e("div", { style: { color: cBad, fontSize: 12, marginBottom: 6 } }, "⚠ 歸檔明細載入失敗：" + sel.error),
						e("button", { style: miniBtn(), onClick: function () { openDetail(ent); } }, "重試"));
				} else {
					const d = sel.detail || {};
					const meta = (d.run && d.run.historyMeta) || {};
					body = e("div", null,
						e("div", { style: { display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" } },
							e("button", { style: miniBtn(), onClick: function () { setSel(null); } }, "← 返回列表"),
							e("span", { style: { color: cMuted, fontSize: 11, fontFamily: mono } }, String(ent.runId || "")),
							d.report == null ? e("span", { style: { color: cWarn, fontSize: 11 } }, "（無報告正文）") : null),
						meta.truncated === true ? e("div", { role: "alert", style: { color: cWarn, fontSize: 11, marginBottom: 6 } },
							"⚠ 此歸檔超過 256KB 保存上限被截斷（丟失明細 " + (meta.droppedFindings || 0) + " 條；計數仍準確）") : null,
						e(ReportView, {
							run: d.run || null,
							text: String(d.report ?? ""),
							badge: "歷史歸檔 · " + fmtDT(ent.archivedAt || ent.endedAt || ent.startedAt),
							filename: "review-" + String(ent.runId || "history") + ".md",
						}));
				}
			} else if (list === null || list.phase === "loading") {
				body = e("div", { style: { color: cMuted, fontSize: 12, padding: "6px 0" } }, "載入歷史記錄…");
			} else if (list.phase === "disabled") {
				body = e("div", { style: { color: cMuted, fontSize: 12, lineHeight: 1.6, padding: "6px 0" } },
					"歷史記錄未啟用：當前為動態插件模式（無檔案系統訪問）。以 bundle/profile 部署後自動啟用；審查閉環本身不受影響。");
			} else if (list.phase === "error") {
				body = e("div", { role: "alert", style: { padding: "6px 0" } },
					e("div", { style: { color: cBad, fontSize: 12, marginBottom: 6 } }, "⚠ 歷史記錄載入失敗：" + list.error),
					e("button", { style: miniBtn(), onClick: loadList }, "重試"));
			} else if ((list.runs || []).length === 0) {
				body = e("div", { style: { color: cMuted, fontSize: 12, lineHeight: 1.6, padding: "6px 0" } },
					"尚無審查歷史——閉環到達終態（通過/達上限/終止/失敗等）後自動歸檔，之後可在這裡回看舊報告與統計。");
			} else {
				body = e("div", { style: { maxHeight: 320, overflowY: "auto", padding: "4px 0" } },
					list.runs.map(function (r) { return historyCard(r, openDetail); }));
			}
			return e(CollapsibleSection, { title: "歷史記錄", subtitle: "終態審查歸檔 · 時間倒序", onOpen: loadList }, body);
		}
		//#endregion
		...
		function ReviewPanel(props) {
			...
			return e("div", { style: { padding: "16px 20px", overflowY: "auto", height: "100%", color: cText, fontSize: 13 } },
				...
				// P1-15：面板「統計」摺疊區（活躍與終態 run 都渲染；數據客戶端自算 publicRun/roundLog/injectLog）
				run ? e(CollapsibleSection, { title: "統計", subtitle: "嚴重度環形 · 輪次收斂 · 維度堆疊 · 檔案熱點 · 頑固項" },
					e(StatsSummary, { run: run })) : null,
				report ? e("div", { style: { marginTop: 14, borderTop: "1px solid " + cBorder, paddingTop: 10 } },
					// P1-7→P1-14：報告視圖——摘要卡+統計摘要+TOC+章節摺疊+複製/下載 MD
					e(ReportView, { run: run, text: String(report) })) : null,
				// P1-13 UI：歷史記錄摺疊區（終態歸檔清單 → 明細只讀回看；與當前 run 無關，閒置也可用）
				e(HistorySection, null),
			);
		}
