...
### P1-7 ✅ 已完成（v1.4.0）— 面板審查匯報美化（用戶反饋 2026-09-02）
...
### P1-8 ✅ 已完成（v1.4.0）— 聚合跨維度去重（驗收分析 2026-09-02）
...
### P1-9 ✅ 已完成（v1.4.0）— 項目級「明確不修」ignore 清單（驗收分析 2026-09-02）
...
### P1-10 ✅ 已完成（v1.4.0）— 審查模型次序控制（用戶反饋 2026-09-02）
...
### P1-11 ✅ 已完成（v1.4.0）— 修復範圍檔位（與通過線解耦）（用戶反饋 2026-09-02）
...
### P1-12 ✅ 已完成（v1.4.0）— 審查頁「每輪修復報告」時間線（用戶反饋 2026-09-02）
...
## v1.4 審查遺留（t4 code review，LOW 記檔）

> v1.4.0 合入前的獨立審查（t4）判定：H-1 已修（.reviewignore pattern/reason 出站清洗 +
> accepted 明細移入 <review-data> 數據塊 + 提示詞數據邊界聲明）；M-1/M-2/M-3 已修
> （dedup 行號窗口±10 與 severity 取最高 / fixScope=all 改 gate 補集 / injectLog items 與
...
- **方案**：
  ...
  ② 寫入時機：任意終態（passed/max-rounds/stopped/oscillated/failed/reported）落盤；每項目 LRU 保留最近 50 次自動修剪；單檔上限 256KB（findings 截斷）。
...
- **方案**（純手寫 SVG/CSS，零圖表庫）：
  ...
  ② **輪次收斂折線**：每輪 mergedCount 隨輪次變化（SVG polyline + 節點數值），一眼看出收斂速度。
  ③ **維度×嚴重度堆疊條**：四維各一橫條，段寬=計數。
  ④ **檔案熱點 Top5**：按 findings 數排序的橫條（哪個檔案問題最集中）。
  ⑤ **重複頑固項**：跨輪重複出現（指紋 streak≥2）的清單——告訴你「哪些問題修不掉」。
...
- **方案**：
  ① 頂部常駐摘要條（狀態/輪次/阻斷/token 即時一眼）。
  ② findings 篩選器（嚴重度/維度/resolved 三維過濾 chips）。
  ③ 審查中骨架/進度（當前輪維度燈 + 已完成審查者計數）。
  ④ 入口升級：`conversation.session.header.actions` 加「▶ 審查」小按鈕（不必切到分頁）。
  ⑤ 當前輪時間線卡默認展開、resume 後自動滾動定位。
  ⑥ 模型排序 ↑↓ → 拖拽（可選）；設置頁分組膠囊導航。
- **工作量**：M（①②③④ 為 S 級單項）。**依賴**：④ 需查 header.actions slot 契約。

### 建議節奏（v1.6 兩批交付）
1. **v1.6.0 第一批**（體感提升最大）：P1-13 歷史 + P1-14 報告美化 + P1-15 統計圖表（三者共用「報告/數據」片區，一次到位）。
2. **v1.6.1 第二批**：P1-16 交互籃子 + 模型拖拽 + 設置導航。
3. 之後回到既有 P2（自動觸發 opt-in、checklist 配置化、跨 run 趨勢聚合）。
