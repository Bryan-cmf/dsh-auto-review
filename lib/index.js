// dsh-auto-review — host half (v1.4.0)
//
...
function parseReviewIgnore(text) {
	...
	for (const raw of String(text ?? '').split('\n')) {
		...
		let fileRe
		try { fileRe = globToRe(fileGlob) } catch { continue }
		rules.push({
...
export function apply(ctx) {
	...
	async function runRound(run) {
		...
		if (run.injectMode === 'manual' || hardFiltered + hardExtra > 0 || hasSecurity) {
			run.status = 'awaiting-confirm'
			run.pendingInject = {
				round: run.round, count, extraCount,
				blockingByDim: safeBlocking, extrasByDim: safeExtras,
				filteredCount: hardFiltered + hardExtra, securityHold: hasSecurity,
