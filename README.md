...
## v1.6 新功能（P1-13 ~ P1-15）

- **審查歷史持久化**（P1-13）：每次閉環到達終態（通過/輪數上限/終止/振盪/失敗/報告模式）
  自動歸檔到 `$DSH_HOME/review-history/<專案>/<runId>/`（`run.json` 完整快照 + `report.md`），
  **不寫被審倉庫**；每專案保留最近 50 次（超額自動修剪最舊）、單檔上限 256KB（超限以
  findings 明細為主截斷，嚴重度計數永不失真並如實標注）。面板「審查」分頁新增**歷史記錄**
  摺疊區：全專案時間倒序卡列表（日期/狀態/輪次/模型/阻斷數/嚴重度計數），點擊即以只讀
  ReportView 回看當時的完整報告與統計（含「歷史歸檔」徽章與截斷警示）。歸檔失敗僅
  `console.error` 靜默降級，絕不影響閉環本身；重啟後歷史仍在。
- **報告渲染升級**（P1-14）：報告區重構為章節化視圖——頂部**摘要卡**（狀態/輪次/token
  in·out/阻斷計數一覽，阻斷數與閉環判定同口徑）＋維度**錨點 TOC**（點擊平滑滾動）＋
  **章節摺疊**（未通過維度默認展開、已通過收起）；Markdown 渲染升級：表格斑馬紋、
  severity 色籤（色點+色字）、行內 code 底色、品牌色引用豎線；右上動作列提供
  **⧉ 複製 MD** 與 **⤓ 下載 .md**（純客戶端，零依賴）。
- **錯誤類型統計圖表**（P1-15）：純手寫 SVG/CSS、零圖表庫——①嚴重度分佈**環形圖**
  ②每輪合併後發現數**收斂折線**（一眼看出修復收斂速度）③維度×嚴重度**堆疊條**
  ④檔案熱點 **Top5** 橫條（問題最集中的檔案）⑤**頑固項清單**（跨輪注入 ≥2 次且仍未
  解決——告訴你哪些問題修不掉）。位置：報告頂部「統計摘要」段＋面板「統計」摺疊區，
  活躍/終態/歷史歸檔三處共用；所有圖表帶 `role=img` + `aria-label` + 文字備援。
