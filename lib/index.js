// dsh-auto-review — host half (v1.6.0)
//
...
//   GET  /__review/api/history?project= → 審查歷史清單（P1-13；slug 可選過濾）
//   GET  /__review/api/history/<runId>  → 審查歷史明細（run.json 快照 + report.md 全文）
//
// v1.1 修復與增強：
...
//
// v1.6 host 增量（P1-13）：
//   審查歷史持久化——終態（passed/max-rounds/stopped/oscillated/failed/reported）歸檔到
//   $DSH_HOME/review-history/<project-slug>/<runId>/（run.json=完整 publicRun 快照 + report.md）；
//   每項目 LRU 保留 50 個、單檔 256KB 截斷（findings 為主，truncated/droppedFindings 如實標注）；
//   API：GET /__review/api/history?project= 清單 + GET /__review/api/history/<runId> 明細
//   （掛既有 x-review-token 鑑權 + Host/Origin 白名單）；歸檔失敗 console.error 靜默降級，絕不影響閉環。
//
...
// P1-13 審查歷史持久化：受信 bundle 代碼直寫 $DSH_HOME/review-history——與 DSH 自家
// dsh-settings-file 寫 settings.yaml 同一 node:fs 通道；不走 ctx.get('fs')（沙箱後端
// 部署預設 read-only，DSH_HOME 寫入會被 FS_SANDBOX_DENIED 拒絕）。動態模式下三行被剝離 →
// nodeApi() 返回 null → 歷史功能靜默停用（閉環不受影響）。
// DYNAMIC-STRIP: 動態插件構建時移除以下三行（沙箱禁止 import；bundle 模式必需）
...
const normTitle = (t) => {
	// L-2（v1.5.0）：正規化為空（純諺文/西里爾等被全剝）時回退原文小寫——避免同檔全碰撞
	const n = String(t ?? '').toLowerCase().replace(/[^0-9a-z\u4e00-\u9fff]+/g, '')
	return n !== '' ? n : String(t ?? '').toLowerCase()
}
...
function globToRe(glob) {
	...
	while (i < src.length) {
		...
		if (c === '*') {
			if (src[i + 1] === '*') {
				// L-5（v1.5.0）：`**/` 譯為段邊界可選組（(?:.*/)?）——`**/test/**` 不再誤匹配 src/contest/；
				//   段尾 `**` 仍為 `.*`（跨目錄通配語義不變）
				if (src[i + 2] === '/') { re += '(?:.*/)?'; i += 3 }
				else { if (!re.endsWith('.*')) re += '.*'; i += 2 }
			} else {
				re += '[^/]*'
				i += 1
			}
...
function matchIgnoreRule(rules, f, root) {
	// R9（S-2 剩餘）：匹配輸入截斷（findings 的 file 無長度上限；防超長輸入放大回溯成本）
	// L-1（v1.5.0）：絕對路徑 finding 先剝 projectPath 前綴再與相對 glob 比對
	let file = String(f?.file ?? '').replace(/^\.\//, '')
	if (root) file = file.replace(new RegExp('^' + String(root).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '/'), '')
	file = file.replace(/^\//, '').slice(0, 512)
...
/** P1-4（v1.5.0）：累加審查者子會話 token 用量（assistant/message.usage；讀取失敗→null 靜默）。 */
	async function sumReviewerUsage(sq, sessionId) {
		if (sq === undefined || typeof sessionId !== 'string' || sessionId === '') return null
		try {
			const snap = await sq.readSession(sessionId)
			let inTok = 0, outTok = 0, seen = false
			for (const ev of snap?.events ?? []) {
				if (ev?.type !== 'assistant/message') continue
				const u = ev.usage ?? ev?.data?.usage
				if (u !== null && typeof u === 'object' && Number.isFinite(u.inputTokens)) {
					inTok += u.inputTokens; outTok += Number(u.outputTokens) || 0; seen = true
				}
			}
			return seen ? { input: inTok, output: outTok } : null
		} catch { return null }
	}
...
function mergedBlockingCount(run) {
	...
	for (const dim of dimList) {
		...
		byDim[dim.id] = (d?.findings ?? []).filter((f) => !f.resolved && run.gateSet.has(f.severity)
			&& matchIgnoreRule(run.reviewIgnores ?? [], f, run.projectPath) === null)
	}
...
// ── P1-13 審查歷史持久化（模組級純函數，便於測試）──────────

/** 歷史根目錄名（$DSH_HOME 之下）；每項目一個子目錄、每 run 一個 <runId>/ 子目錄。 */
const HISTORY_DIR_NAME = 'review-history'
/** 每項目 LRU 保留的 run 數（按 archivedAt 新→舊，超額逐出最舊並刪目錄）。 */
const HISTORY_KEEP_PER_PROJECT = 50
/** 單檔（run.json / report.md）位元組預算：超過則截斷（findings 為主要削減對象）。 */
const HISTORY_FILE_BUDGET_BYTES = 256 * 1024
/** 清單端點單次返回的 run 摘要上限（跨項目合併後；防面板一次拉爆）。 */
const HISTORY_LIST_MAX = 300
/** 歸檔的終態集合——paused/interrupted/awaiting-confirm 為可恢復暫態，不歸檔。 */
const HISTORY_ARCHIVED_STATUSES = new Set(['passed', 'max-rounds', 'stopped', 'oscillated', 'failed', 'reported'])
/** runId / project-slug 白名單（API 路徑參數與落盤目錄名共用；防路徑穿越）。 */
const HISTORY_SAFE_ID = /^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/

/** 受信 node 綁定的惰性取用：動態模式（import 被 DYNAMIC-STRIP 剝離）返回 null → 歷史停用。
 *  惰性求值 + try/catch：即使剝離構建被誤以非動態環境載入，也不會在模組載入時拋 ReferenceError。 */
let _nodeApi = null
let _nodeApiResolved = false
function nodeApi() {
	if (!_nodeApiResolved) {
		_nodeApiResolved = true
		if (!IS_DYNAMIC) {
			try { _nodeApi = { mkdir, readdir, readFile, rename, rm, writeFile, homedir, join } } catch { _nodeApi = null }
		}
	}
	return _nodeApi
}

/** $DSH_HOME 解析——語義與宿主 @deepseek-ai/dsh-home-paths 的 resolveDshHome 對齊：
 *  $DSH_HOME 環境變量（trim 後非空才生效，空值視為未設置）→ 回退 ~/.dsh。
 *  首次解析後緩存（進程內不變）；無 node 綁定（動態模式）返回 null。 */
let _dshHomeCache = null
let _dshHomeResolved = false
function dshHomeDir() {
	if (!_dshHomeResolved) {
		_dshHomeResolved = true
		const api = nodeApi()
		if (api === null) { _dshHomeCache = null }
		else {
			try {
				const env = typeof process !== 'undefined' && process && process.env ? String(process.env.DSH_HOME ?? '') : ''
				const trimmed = env.trim()
				_dshHomeCache = trimmed !== '' ? trimmed : api.join(api.homedir(), '.dsh')
			} catch { _dshHomeCache = null }
		}
	}
	return _dshHomeCache
}

/** 專案路徑 → 歷史 slug：basename 經 [A-Za-z0-9._-] 白名單清洗（剝前後標點、去重分隔符），
 *  ≤64 字符；空/退化結果回退 'project'。路徑穿越不可能：'/' 已被 basename 切除、'..' 會被
 *  前後標點剝離歸一為空 → 'project'。 */
function historySlugOf(projectPath) {
	const base = String(projectPath ?? '').split('/').filter(Boolean).pop() ?? ''
	const slug = base.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^[.-]+|[.-]+$/g, '').slice(0, 64).replace(/^[.-]+|[.-]+$/g, '')
	return slug !== '' ? slug : 'project'
}

/** UTF-8 位元組長度（node Buffer；剝離環境退化為 UTF-16 估計——僅影響截斷精度不影響正確性）。 */
function byteLenOf(s) {
	return typeof Buffer !== 'undefined' && typeof Buffer.byteLength === 'function' ? Buffer.byteLength(s) : s.length * 2
}

/** P1-13：run.json 256KB 預算擬合——以「findings 為主」的次序漸進削減（每階段後重新量測）：
 *  ① 文本欄位收緊（detail/suggestion→400、title→160、summary/error→500、ignoredByDecision 截 30）
 *  ② injectLog 逐條 items/extraItems 截 20
 *  ③ 丟 low findings → ④ 丟 medium → ⑤ roundLog 留 8 / injectLog 留 10
 *  ⑥ 丟 high → ⑦ roundLog 留 3 / injectLog 留 5 且 items 清空 → ⑧ 丟 critical
 *  ⑨ 骨架化（findings 全清）→ ⑩ 最小對象兜底（理論不可達）
 *  被削減即置 historyMeta.truncated=true 並累計 droppedFindings（僅統計整條丟失）。
 *  輸入 view 不被改動（內部深拷貝）；返回 { text, truncated, droppedFindings }。 */
function fitHistoryRunJson(view, budgetBytes) {
	const direct = JSON.stringify(view)
	if (byteLenOf(direct) <= budgetBytes) return { text: direct, truncated: false, droppedFindings: 0 }
	const obj = JSON.parse(direct) // 深拷貝（plain 數據；publicRun 輸出無活引用）
	let dropped = 0
	const fits = () => byteLenOf(JSON.stringify(obj)) <= budgetBytes
	const dropSeverity = (sev) => {
		for (const d of obj.dimensions ?? []) {
			const before = (d.findings ?? []).length
			d.findings = (d.findings ?? []).filter((f) => f.severity !== sev)
			dropped += before - d.findings.length
		}
	}
	// ① 文本欄位收緊
	for (const d of obj.dimensions ?? []) {
		d.summary = String(d.summary ?? '').slice(0, 500)
		d.error = String(d.error ?? '').slice(0, 500)
		for (const f of d.findings ?? []) {
			f.title = String(f.title ?? '').slice(0, 160)
			f.detail = String(f.detail ?? '').slice(0, 400)
			f.suggestion = String(f.suggestion ?? '').slice(0, 400)
		}
	}
	if (obj.error != null) obj.error = String(obj.error).slice(0, 500)
	obj.ignoredByDecision = Object.fromEntries(Object.entries(obj.ignoredByDecision ?? {})
		.map(([k, arr]) => [k, (Array.isArray(arr) ? arr : []).slice(0, 30)]))
	if (fits()) return done()
	// ② injectLog 條目快照截 20
	for (const e of obj.injectLog ?? []) {
		if (Array.isArray(e.items)) e.items = e.items.slice(0, 20)
		if (Array.isArray(e.extraItems)) e.extraItems = e.extraItems.slice(0, 20)
	}
	if (fits()) return done()
	// ③→⑧ 漸進丟 findings / 裁時間線
	dropSeverity('low'); if (fits()) return done()
	dropSeverity('medium'); if (fits()) return done()
	if (Array.isArray(obj.roundLog)) obj.roundLog = obj.roundLog.slice(-8)
	if (Array.isArray(obj.injectLog)) obj.injectLog = obj.injectLog.slice(-10)
	if (fits()) return done()
	dropSeverity('high'); if (fits()) return done()
	if (Array.isArray(obj.roundLog)) obj.roundLog = obj.roundLog.slice(-3)
	if (Array.isArray(obj.injectLog)) {
		obj.injectLog = obj.injectLog.slice(-5)
		for (const e of obj.injectLog) { e.items = []; e.extraItems = [] }
	}
	if (fits()) return done()
	dropSeverity('critical'); if (fits()) return done()
	// ⑨ 骨架化（counts 仍在——數量不失真，僅失明細）
	for (const d of obj.dimensions ?? []) { d.findings = []; d.ignored = [] }
	obj.ignoredByDecision = {}
	if (fits()) return done()
	// ⑩ 最小對象兜底（保留標識與終態，丟一切明細）
	return {
		text: JSON.stringify({ runId: obj.runId, sessionId: obj.sessionId, projectPath: obj.projectPath, status: obj.status, round: obj.round, maxRounds: obj.maxRounds, startedAt: obj.startedAt, historyMeta: { ...(obj.historyMeta ?? {}), truncated: true, droppedFindings: dropped } }),
		truncated: true, droppedFindings: dropped,
	}
	function done() {
		if (obj.historyMeta == null || typeof obj.historyMeta !== 'object') obj.historyMeta = {}
		obj.historyMeta.truncated = true
		obj.historyMeta.droppedFindings = dropped
		return { text: JSON.stringify(obj), truncated: true, droppedFindings: dropped }
	}
}

/** P1-13：report.md 256KB 預算擬合（純文本可安全硬截；尾部標注截斷）。 */
function fitHistoryText(text, budgetBytes) {
	const s = String(text ?? '')
	if (byteLenOf(s) <= budgetBytes) return { text: s, truncated: false }
	let t = s
	while (byteLenOf(t) > budgetBytes - 160 && t.length > 0) t = t.slice(0, Math.max(0, t.length - 1024))
	return { text: `${t}\n\n> ⚠️ 報告超過 256KB 保存上限，已截斷。`, truncated: true }
}

/** P1-13：run → 清單摘要條目（index.json 與 API 共用；計數取歸檔時的未收斂口徑）。 */
function historySummaryOf(run, truncated, droppedFindings, archivedAt) {
	...
	return {
		...
		project: historySlugOf(run.projectPath),
		projectName: String(run.projectPath ?? '').split('/').filter(Boolean).pop() ?? '',
		projectPath: run.projectPath,
		sessionId: run.sessionId,
		status: run.status, mode: run.mode, injectMode: run.injectMode,
		gate: run.gate, fixScope: run.fixScope ?? DEFAULT_FIX_SCOPE, scope: run.scope,
		round: run.round, maxRounds: run.maxRounds,
		startedAt: run.startedAt, endedAt: run.endedAt ?? null,
		blocking: run.dims ? mergedBlockingCount(run) : 0,
		severityCounts: sev,
		injectCount: run.injectLog?.length ?? 0,
		models: (run.models ?? []).map((m) => m.model),
		truncated: truncated === true,
		droppedFindings: Number(droppedFindings) || 0,
		archivedAt: Number(archivedAt) || null,
		hasReport: true,
	}
}

/** P1-13：已落盤的 run.json 文檔 → 清單摘要（index 壞損自愈重建用；計數取文檔 counts）。 */
function historySummaryFromDoc(doc) {
	const sev = { critical: 0, high: 0, medium: 0, low: 0 }
	for (const d of doc.dimensions ?? []) {
		for (const [k, n] of Object.entries(d?.counts ?? {})) if (k in sev) sev[k] += Number(n) || 0
	}
	const rl = Array.isArray(doc.roundLog) ? doc.roundLog : []
	const meta = doc.historyMeta ?? {}
	return {
		runId: String(doc.runId ?? ''),
		project: typeof meta.project === 'string' ? meta.project : historySlugOf(doc.projectPath),
		projectName: String(doc.projectPath ?? '').split('/').filter(Boolean).pop() ?? '',
		projectPath: doc.projectPath,
		sessionId: doc.sessionId,
		status: doc.status, mode: doc.mode, injectMode: doc.injectMode,
		gate: doc.gate, fixScope: doc.fixScope, scope: doc.scope,
		round: doc.round, maxRounds: doc.maxRounds,
		startedAt: doc.startedAt, endedAt: doc.endedAt ?? null,
		blocking: rl.length > 0 ? (rl[rl.length - 1].mergedCount ?? null) : null,
		severityCounts: sev,
		injectCount: Array.isArray(doc.injectLog) ? doc.injectLog.length : 0,
		models: Array.isArray(doc.models) ? doc.models : [],
		truncated: meta.truncated === true,
		droppedFindings: Number(meta.droppedFindings) || 0,
		archivedAt: Number(meta.archivedAt) || Number(doc.endedAt) || null,
		hasReport: true,
	}
}
...
function initDims(dimList) {
	...
	for (const d of dimList) {
		dims[d.id] = {
			...
			findings: [], summary: '', reviewedFiles: [], lastRunAt: null, error: null, tokenUsage: null, // P1-4：該輪此維度審查者 token 用量
		}
	}
	return dims
}
...
function buildReport(run) {
	...
	out.push('---')
	{ // P1-4：token 成本行（有數據才顯示）
		let ti = 0, to = 0, seen = false
		for (const dim of run.dimList) {
			const u = run.dims[dim.id]?.tokenUsage
			if (u) { ti += u.input; to += u.output; seen = true }
		}
		if (seen) out.push(`- 累計 token（審查者，近似）：輸入 ${ti.toLocaleString()} / 輸出 ${to.toLocaleString()}`)
	}
...
export function apply(ctx) {
	...
	function snapshotOf(run) {
		...
		return {
			...
			// L-3（v1.5.0）：已接受分組隨快照保留（每維截 30 條精簡欄位）——重啟恢復輪不失「已接受」展示
			ignoredByDecision: Object.fromEntries(Object.entries(run.ignoredByDecision ?? {}).map(([k, arr]) =>
				[k, (Array.isArray(arr) ? arr : []).slice(0, 30).map((f) => ({
					severity: f.severity, file: f.file, line: f.line ?? null, title: String(f.title ?? '').slice(0, 120),
					ignorePattern: f.ignorePattern, ignoreReason: f.ignoreReason,
				}))])),
	...
	function hydrateSnapshot(snap) {
		...
		const run = {
			...
			ignoredByDecision: (snap.ignoredByDecision && typeof snap.ignoredByDecision === 'object') ? snap.ignoredByDecision : {}, // L-3：已接受分組恢復
	...
	// ── P1-13：審查歷史持久化（$DSH_HOME/review-history；bundle 模式；動態模式靜默停用）──
	let historyOffNotified = false
	function historyOffNotice() {
		if (historyOffNotified) return
		historyOffNotified = true
		console.error(`[${PLUGIN_TAG}] 審查歷史持久化未啟用（動態插件模式無 node fs 綁定；bundle/profile 模式自動啟用）`)
	}
	/** 歷史根目錄（緩存；node 綁定缺席 → null = 功能停用）。 */
	let _historyRootCache
	function historyRootDir() {
		if (_historyRootCache === undefined) {
			const api = nodeApi()
			const home = dshHomeDir()
			_historyRootCache = api === null || home === null ? null : api.join(home, HISTORY_DIR_NAME)
		}
		return _historyRootCache
	}
	/** 歷史寫入串行鏈：同進程內 index.json 讀-改-寫不互踩（與快照鏈獨立，互不阻塞）。 */
	...
	/** 臨時檔 + rename 原子落盤（崩潰不留下半寫的 run.json/index.json）。 */
	async function historyWriteFile(api, file, content) {
		const tmp = `${file}.tmp-${Math.random().toString(36).slice(2, 8)}`
		await api.writeFile(tmp, content, 'utf8')
		await api.rename(tmp, file)
	}
	/** 讀項目 index.json；缺席/壞損 → 掃描各 runId 目錄的 run.json 自愈重建（僅壞損時走此重路徑）。 */
	async function historyReadIndex(api, root, slug) {
		try {
			const parsed = JSON.parse(await api.readFile(api.join(root, slug, 'index.json'), 'utf8'))
			if (parsed !== null && typeof parsed === 'object' && Array.isArray(parsed.runs)) {
				return {
					schema: 1, updatedAt: parsed.updatedAt ?? null,
					runs: parsed.runs.filter((x) => x !== null && typeof x === 'object' && HISTORY_SAFE_ID.test(String(x.runId ?? ''))),
				}
			}
		} catch {} // 缺席/壞損 → 重建
		const runs = []
		try {
			for (const name of (await api.readdir(api.join(root, slug))).filter((n) => HISTORY_SAFE_ID.test(n))) {
				if (name === 'index.json') continue
				try {
					const doc = JSON.parse(await api.readFile(api.join(root, slug, name, 'run.json'), 'utf8'))
					if (doc !== null && typeof doc === 'object' && doc.runId !== undefined) runs.push(historySummaryFromDoc(doc))
				} catch {}
			}
		} catch {}
		runs.sort((a, b) => (b.archivedAt ?? 0) - (a.archivedAt ?? 0))
		return { schema: 1, updatedAt: null, runs: runs.slice(0, HISTORY_KEEP_PER_PROJECT) }
	}
	/** 終態 run → 歸檔（best-effort 鏈上執行；任何失敗 console.error 帶標籤靜默降級，絕不影響閉環）。 */
	function archiveRunHistory(run) {
		const api = nodeApi()
		const root = historyRootDir()
		if (api === null || root === null) { historyOffNotice(); return }
		const runId = String(run.runId ?? '')
		if (!HISTORY_SAFE_ID.test(runId)) {
			console.error(`[${PLUGIN_TAG}] 審查歷史歸檔跳過：runId 不合法（${runId.slice(0, 40)}）`)
			return
		}
		const slug = historySlugOf(run.projectPath)
		void enqueueHistory(async () => {
			try {
				const archivedAt = Date.now()
				const dir = api.join(root, slug, runId)
				// run.json = 完整 publicRun 快照 + historyMeta（擬合後如實標注截斷）
				const doc = { ...publicRun(run), historyMeta: { schema: 1, archivedAt, project: slug, truncated: false, droppedFindings: 0 } }
				const fitted = fitHistoryRunJson(doc, HISTORY_FILE_BUDGET_BYTES)
				await api.mkdir(dir, { recursive: true })
				await historyWriteFile(api, api.join(dir, 'run.json'), fitted.text + '\n')
				const repFit = fitHistoryText(buildReport(run), HISTORY_FILE_BUDGET_BYTES)
				await historyWriteFile(api, api.join(dir, 'report.md'), repFit.text.endsWith('\n') ? repFit.text : repFit.text + '\n')
				// index.json 讀-改-寫 + LRU 逐出（新→舊排序，超额刪最舊目錄；永不逐出本次）
				const idx = await historyReadIndex(api, root, slug)
				const runs = idx.runs.filter((x) => String(x.runId ?? '') !== runId)
				runs.push({ ...historySummaryOf(run, fitted.truncated, fitted.droppedFindings, archivedAt) })
				runs.sort((a, b) => (b.archivedAt ?? 0) - (a.archivedAt ?? 0))
				const evicted = runs.splice(HISTORY_KEEP_PER_PROJECT)
				await historyWriteFile(api, api.join(root, slug, 'index.json'), JSON.stringify({ schema: 1, updatedAt: archivedAt, runs }) + '\n')
				for (const e of evicted) {
					if (e == null || String(e.runId ?? '') === runId || !HISTORY_SAFE_ID.test(String(e.runId ?? ''))) continue
					try { await api.rm(api.join(root, slug, String(e.runId)), { recursive: true, force: true }) } catch {}
				}
			} catch (err) {
				console.error(`[${PLUGIN_TAG}] 審查歷史歸檔失敗（靜默降級，不影響閉環）`, err)
			}
		})
	}
	/** 終態歸檔鉤子：僅六種終態歸檔；paused/interrupted/awaiting-confirm 可恢復，不歸檔。 */
	function archiveIfTerminal(run) {
		if (!HISTORY_ARCHIVED_STATUSES.has(run.status)) return
		archiveRunHistory(run)
	}
	/** 清單端點載荷：?project=<slug> 過濾單項目；缺省跨項目合併（archivedAt 新→舊，截 HISTORY_LIST_MAX）。
	 *  讀取掛 historyChain——FIFO 排在進行中/排隊的歸檔之後，讀到的一定包含已觸發的終態（無競態空窗）。 */
	async function historyListPayload(projectFilter) {
		const api = nodeApi()
		const root = historyRootDir()
		if (api === null || root === null) { historyOffNotice(); return { ok: true, disabled: true, projects: [], runs: [] } }
		const raw = String(projectFilter ?? '').trim()
		const project = raw !== '' && HISTORY_SAFE_ID.test(raw) ? raw : ''
		return enqueueHistory(async () => {
			let slugs = []
			try { slugs = (await api.readdir(root)).filter((n) => HISTORY_SAFE_ID.test(n) && n !== 'index.json') } catch { return { ok: true, projects: [], runs: [] } }
			const runs = []
			for (const s of (project !== '' ? slugs.filter((x) => x === project) : slugs)) {
				const idx = await historyReadIndex(api, root, s)
				for (const r of idx.runs) runs.push({ ...r, project: s })
			}
			runs.sort((a, b) => (b.archivedAt ?? 0) - (a.archivedAt ?? 0))
			return { ok: true, projects: slugs, runs: runs.slice(0, HISTORY_LIST_MAX) }
		})
	}
	/** 明細端點載荷：直接讀 <slug>/<runId>/run.json（不依賴 index）；?project= 可選限定掃描範圍。
	 *  同樣掛 historyChain（與清單一致的讀寫序）。 */
	async function historyGetPayload(runId, projectFilter) {
		const api = nodeApi()
		const root = historyRootDir()
		if (api === null || root === null) { historyOffNotice(); return { ok: false, disabled: true, error: '審查歷史未啟用（需 bundle 模式）' } }
		if (!HISTORY_SAFE_ID.test(runId)) return { ok: false, error: '非法 runId' }
		const raw = String(projectFilter ?? '').trim()
		const project = raw !== '' && HISTORY_SAFE_ID.test(raw) ? raw : ''
		return enqueueHistory(async () => {
			let slugs = []
			try { slugs = (await api.readdir(root)).filter((n) => HISTORY_SAFE_ID.test(n) && n !== 'index.json') } catch { return { ok: false, error: '找不到該審查歷史' } }
			for (const s of (project !== '' ? slugs.filter((x) => x === project) : slugs)) {
				let doc = null
				try { doc = JSON.parse(await api.readFile(api.join(root, s, runId, 'run.json'), 'utf8')) } catch {}
				if (doc === null || typeof doc !== 'object') continue
				let report = null
				try { report = await api.readFile(api.join(root, s, runId, 'report.md'), 'utf8') } catch {}
				return { ok: true, project: s, run: doc, report }
			}
			return { ok: false, error: '找不到該審查歷史' }
		})
	}
	...
	function finish(run) {
		...
		archiveIfTerminal(run) // P1-13：終態歸檔（best-effort 異步；非終態自動跳過）
	}
	...
	function stopRun(sessionId, reason = 'user') {
		...
		if (run === undefined) {
			...
			if (last !== undefined && (last.status === 'awaiting-confirm' || last.status === 'interrupted' || last.status === 'paused')) {
				...
				archiveIfTerminal(last) // P1-13：重啟恢復態被明確放棄 → 'stopped' 終態歸檔（未經 finish()）
	...
	async function reviewDimension(run, dim, parent, prevBlocking, changedFiles) {
		...
		try {
			...
			d.pass = blockingOf(d.findings, run.gateSet)
				.filter((f) => matchIgnoreRule(run.reviewIgnores ?? [], f, run.projectPath) === null).length === 0
			d.status = d.pass ? 'passed' : 'blocking'
			d.lastRunAt = Date.now()
			d.tokenUsage = await sumReviewerUsage(sessionQuery, String(started?.id ?? '')) // P1-4：本輪此維度 token 用量
			return d
		} finally {
			killTimer()
	...
	async function runRound(run) {
		...
		for (const dim of run.dimList) {
			...
			for (const f of (d.findings ?? []).filter((x) => !x.resolved && run.gateSet.has(x.severity))) {
				const rule = matchIgnoreRule(run.reviewIgnores ?? [], f, run.projectPath)
		...
		run.roundLog.push({
			...
			tokenUsage: (() => { // P1-4：本輪各維審查者 token 合計
				let ti = 0, to = 0, seen = false
				for (const dim of run.dimList) {
					const u = run.dims[dim.id]?.tokenUsage
					if (u) { ti += u.input; to += u.output; seen = true }
				}
				return seen ? { input: ti, output: to } : null
			})() })
	...
	if (IS_DYNAMIC && typeof harness.handle === 'function') {
		...
		rpc('review-history-list', (args) => historyListPayload(String(args.project ?? '')))
		rpc('review-history-get', (args) => {
			const rid = String(args.runId ?? '')
			if (rid === '' || !HISTORY_SAFE_ID.test(rid)) return { ok: false, error: '非法 runId' }
			return historyGetPayload(rid, String(args.project ?? ''))
		})
	...
	ctx.effect(() => {
		const reg = ctx.webServer.register({
			...
			handler: (req, res) => {
				...
				// P1-13：審查歷史——清單與明細（與其他端點同一 Host 白名單 + x-review-token 鑑權；
			// 遠端響應沿用 projectPath 鍵脫敏 + 報告正文絕對路徑替換）
			if (req.method === 'GET' && p === '/__review/api/history') {
				safeAsync(async () => {
					const body = await historyListPayload(query.project ?? '')
					send(200, maskPaths ? maskBody(body) : body)
				})
				return
			}
			if (req.method === 'GET' && p.startsWith('/__review/api/history/')) {
				safeAsync(async () => {
					let runId = ''
					try { runId = decodeURIComponent(p.slice('/__review/api/history/'.length)) } catch { runId = '' }
					if (runId === '' || !HISTORY_SAFE_ID.test(runId)) return send(400, { ok: false, error: '非法 runId' })
					const body = await historyGetPayload(runId, query.project ?? '')
					if (body.ok !== true) return send(404, body)
					// 報告正文含絕對項目路徑（buildReport 標題行）——遠端先替換再整體脫敏
					if (maskPaths && body.run?.projectPath != null && typeof body.report === 'string') {
						body.report = body.report.replaceAll(body.run.projectPath, maskAbs(body.run.projectPath))
					}
					send(200, maskPaths ? maskBody(body) : body)
				})
				return
	...
	function publicRun(run) {
		return {
			...
			tokenUsage: (() => { // P1-4：全閉環累計（各維最新一輪之和）
				let ti = 0, to = 0, seen = false
				for (const dim of run.dimList) {
					const u = run.dims[dim.id]?.tokenUsage
					if (u) { ti += u.input; to += u.output; seen = true }
